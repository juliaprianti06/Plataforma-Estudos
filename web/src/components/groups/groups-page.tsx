...
export function GroupsPage() {
  ...
  return session ? <GroupsContent key={session.mode + session.user.id + session.accessToken} session={session} /> : <Navigate to="/" replace />
}
...
function GroupsContent({ session }: { session: AuthSession }) {
  ...
  const repository = useMemo(() => session.mode === 'api' ? createApiGroupsRepository(session) : createLocalGroupsRepository(session.user.id, {
        ...
        <div className="mx-auto max-w-[1440px] px-4 py-5 sm:px-6 sm:py-8 xl:px-8">
          <header className="mb-6 flex flex-col gap-5 lg:flex-row lg:items-center lg:justify-between">
            ...
            <div className="flex flex-wrap items-center gap-2.5">
              ...
              <Button disabled={loading} variant="outline" className="h-9 flex-1 rounded-full border-accent/30 bg-transparent px-3 text-[11px] text-accent sm:flex-none" onClick={() => setDiscover(true)}><Plus className="size-3.5" /> Adicionar grupo</Button>
