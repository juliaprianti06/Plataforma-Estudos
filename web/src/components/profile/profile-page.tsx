...
import { getProfileStore, useProfile, useProfileStatus } from '@/profile/use-profile'
...
export function ProfilePage() {
  ...
  if (!profile) return (
    <main className="grid min-h-screen place-items-center bg-background p-6 text-primary">
      <div className="space-y-4 text-center">
        <p role={status.error ? 'alert' : 'status'}>{status.error ?? 'Carregando seu perfil...'}</p>
        {status.error && <Button type="button" onClick={() => void getProfileStore(session).load().catch(() => {})}>Tentar novamente</Button>}
      </div>
    </main>
  )
  return <ProfileContent key={session.mode + session.user.id + session.accessToken} session={session} profile={profile} />
}
...
function ProfileContent({ session, profile }: { session: AuthSession; profile: UserProfile }) {
  ...
  return (
    <div className="flex min-h-screen bg-background text-foreground">
          ...
          <div className="grid items-start gap-5 xl:grid-cols-[180px_minmax(0,1fr)] xl:gap-7">
            <nav aria-label="Configurações da conta" className="flex gap-1 rounded-2xl border border-border bg-card/60 p-1.5 xl:flex-col xl:gap-2 xl:border-0 xl:bg-transparent xl:p-0">
              {tabs.map(({ id, label, detail, icon: Icon }) => (
                <button key={id} type="button" aria-current={tab === id ? 'page' : undefined} disabled={photoBusy || saving} onClick={() => { setTab(id); setError('') }} className={cn('flex min-w-0 flex-1 items-center justify-center gap-2 rounded-xl px-3 py-3 text-xs transition focus-visible:outline-2 focus-visible:outline-ring xl:justify-start', tab === id ? 'bg-muted font-semibold text-accent' : 'text-muted-foreground hover:bg-card')}>
            ...
            <div className="min-w-0">
              <form onSubmit={save} className="overflow-hidden rounded-[20px] border border-border bg-card shadow-sm shadow-primary/10" aria-busy={saving || photoBusy}>
                <fieldset disabled={saving} className="min-w-0">
                  ...
                  {tab === 'account' && <AccountPanel session={session} onReset={() => setResetOpen(true)} />}
                </fieldset>
              ...
              <p className="mt-4 text-center text-[10px] leading-relaxed text-muted-foreground">{profile.updatedAt ? `Última atualização em ${new Intl.DateTimeFormat('pt-BR', { dateStyle: 'short', timeStyle: 'short' }).format(new Date(profile.updatedAt))} · ` : ''}{session.mode === 'api' ? 'Personalização salva na sua conta.' : 'Personalização salva neste navegador.'}</p>
            </div>
          </div>
        </div>
      </main>
      ...
      <Dialog open={resetOpen} onOpenChange={(open) => { if (!saving) setResetOpen(open) }}>
        <DialogContent className="p-6 sm:max-w-md">
          ...
          <div className="mt-2 flex justify-end gap-2"><Button type="button" variant="outline" className="rounded-full" disabled={saving} onClick={() => setResetOpen(false)}>Cancelar</Button><Button type="button" className="rounded-full" disabled={saving} onClick={() => void reset()}>{saving ? 'Restaurando...' : 'Restaurar perfil'}</Button></div>
        </DialogContent>
      </Dialog>
